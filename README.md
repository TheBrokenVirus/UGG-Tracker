...
**Personal bests** — if a member has ever set a new single-day or single-week XP record, small "Best Day X" / "Best Week Y" pill chips show up under their rank. Both are tracked automatically, not something anyone has to set: best-day is the most XP gained between one checkin and the next on a given calendar day (UTC), best-week is the highest weekly total a completed week has ever closed with. A brand-new member with no records yet simply doesn't get the chips at all, rather than showing "Best Day 0."

**Lift badges** — small DL/BP/SQ badges stacked under the tier badge (or in its place, if there's no tier), showing whatever's been recorded with `/setlift`. Only appears once at least one lift has been set — not a permanent row of dashes for members not using the feature.
